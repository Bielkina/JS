...
function f2() {
    ...
    // let divOrange = document.createElement('div')
    // divOrange.classList.add('bg-orange')
...
function f5() {
    ...
    let bg = document.querySelector('.out-4')
    console.log(bg);
    if (bg.classList == 'out-4 bg-orange') {
        out.innerText = ('true')
        console.log(out);
    } else {
        out.innerText = ('false')
...
function f6() {
    ...
    out.textContent = p6.length

    // for (let i = 1; i <= p6.length; i++){
    //     out.append(i + ' ')
    // }
...
div9.onclick = f9();
// div9.addEventListener('click', f9()) 

// div9.innerHTML = ''
// for (i = 0; i < div9.length; i++){
//     div9[i].addEventListener('click', f9)
// }
...
function f16() {
    ...
    out.parentNode.insertAfter(div, out)
}
...
function f18() {
    ...
    let dataB = p18.getAttribute('data-b')
    out.append(dataB)
}
...
function f19() {
    ...
    p19.forEach(item => {
        console.log(item.getAttribute('data-b'));
        out.append(item.getAttribute('data-b') + ' ')
    })
}
